...
Also keep a concise `Change history` that dates meaningful additions or revalidation. This lets an agent distinguish an old but still-reviewed rule from guidance that has not been checked against the current repository for a long time.
...
- When new operational knowledge is appended to an existing file, add a dated `Change history` entry describing the addition.
...
- 2026-08-27 — Added the Tachyon Skill catalog and repository placement rule.
- 2026-08-27 — Added explicit freshness metadata and dated change-history conventions for Agent Skills.
