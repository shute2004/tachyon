...
mod tests {
    ...
    fn uri_deny_and_metadata_matcher_use_executor_paths() {
        ...
        assert!(!FileSystemSandboxPolicy::matches_prepared_read_deny(
            &path("file:///C:/workspace/public.txt"),
            &context,
            &prepared,
        ));
        // These malformed/opaque candidates intentionally fail closed rather
        // than exercising ordinary Windows glob matching.
    ...
    fn uri_deny_globs_use_executor_home() {
        for (cwd, home, pattern, candidate, outside_home) in [
            (
                "file:///workspace",
                "file:///home/executor",
                "~/private/*.env",
                "file:///home/executor/private/secret.env",
                "file:///home/other/private/secret.env",
            ),
            (
                "file:///workspace",
                "file:///home/executor",
            ...
            (
                ...
                "file:///C:/Users/other/private/secret.env",
            ),
            (
                "file:///C:/workspace",
                "file:///C:/Users/executor",
            ...
            let outside_home = PathUri::parse(outside_home).expect("outside-home path");
            let context = FileSystemSandboxPolicyContext {
                cwd: &cwd,
                workspace_roots: std::slice::from_ref(&cwd),
    ...
    #[test]
    fn uri_posix_glob_matches_decoded_non_utf8_path_bytes() {
        let path = |path| PathUri::parse(path).expect("valid path URI");
        let cwd = path("file:///workspace");
        let context = FileSystemSandboxPolicyContext {
            cwd: &cwd,
        ...
        let prepared = policy
            .prepare_deny_read_matcher(&context, InvalidDenyReadGlobBehavior::ReturnError)
            .expect("POSIX deny matcher");

        assert!(FileSystemSandboxPolicy::matches_prepared_read_deny(
            &path("file:///workspace/secret%FF.env"),
            &context,
            &prepared,
        ));
        assert!(!FileSystemSandboxPolicy::matches_prepared_read_deny(
            &path("file:///workspace/public%FF.txt"),
            &context,
            &prepared,
        ));
    }
