...
/// Constructs the default attachment store that preserves attachments inline.
pub fn inline_attachment_store() -> Arc<dyn AttachmentStore> {
...
impl ThreadManager {
    ...
    pub(crate) fn with_models_provider_home_and_state_for_tests(
        ...
        Self {
            state: Arc::new(ThreadManagerState {
                ...
                attachment_store: inline_attachment_store(),
                thread_store,
                agent_graph_store,
    ...
    /// Returns the attachment store used by threads created by this manager.
    pub fn attachment_store(&self) -> Arc<dyn AttachmentStore> {
        Arc::clone(&self.state.attachment_store)
    }
