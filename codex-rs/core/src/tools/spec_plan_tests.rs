...
use crate::tools::collect_tool_namespaces_info_for_router;
...
impl ToolPlanProbe {
    ...
    fn from_router_with_namespace_info(
        router: ToolRouter,
        include_tool_namespaces_info: bool,
    ) -> Self {
        let tool_namespaces_info = include_tool_namespaces_info
            .then(|| collect_tool_namespaces_info_for_router(&router))
            .filter(|info| !info.is_empty());
        let mut probe = Self::from_router(router);
        probe.tool_namespaces_info = tool_namespaces_info;
        probe
    }
...
async fn probe_with(
    ...
    let include_tool_namespaces_info = turn.config.tool_registry.turn_metadata_includes_tool_info
        && turn.model_info().use_responses_lite;
    ToolPlanProbe::from_router_with_namespace_info(
        plan_with_model(&turn, turn.model_info(), inputs),
...
async fn candidate_model_plan_leaves_selected_model_and_inventory_unchanged() {
    ...
    let selected = ToolPlanProbe::from_router_with_namespace_info(
        plan_with_model(&turn, turn.model_info(), ToolPlanInputs::default()),
        true,
    );
    ...
    let candidate = ToolPlanProbe::from_router_with_namespace_info(
        plan_with_model(&turn, &candidate_model, ToolPlanInputs::default()),
        true,
    );
